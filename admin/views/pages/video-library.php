<?php
use WP_Arvan\Engine\VOD\Assets\Videos_Table;
...
if ( 'grid' === $mode ) {
	...
	$q = $_GET;
	Helper::sanitize_recursive($q,'sanitize_text_field');
	...
	$title       = __( 'Video Library', 'arvancloud-vod' );
	$parent_file = 'admin.php?page=arvancloud-vod-videos';
	...
	<div class="wrap" id="wp-media-grid" data-search="<?php _admin_search_query(); ?>">
		...
		if ( current_user_can( 'upload_files' ) ) {
			?>
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=arvancloud-vod-videos-add' ) ); ?>" class="page-title-action aria-button-if-js"><?php echo esc_html_x( 'Add New', 'file' ); ?></a>
			<?php
		}
		?>
		...
		<div class="error hide-if-js">
			...
			printf(
				...
				'admin.php?page=arvancloud-vod-videos&mode=list'
			);
			?>
			</p>
		</div>
...
$wp_list_table = new Videos_Table();
...
if ( $doaction ) {
	...
	if ( 'delete_all' === $doaction ) {
		...
		Helper::sanitize_recursive($post_ids, 'absint');
	} elseif ( isset( $_REQUEST['ids'] ) ) {
		$post_ids = explode( ',', $_REQUEST['ids'] );
		Helper::sanitize_recursive($post_ids,'absint');
	}

	$location = 'admin.php?page=arvancloud-vod-videos';
	$referer  = wp_get_referer();
	if ( $referer ) {
		if ( false !== strpos( $referer, 'admin.php?page=arvancloud-vod-videos' ) ) {
	...
	switch ( $doaction ) {
		case 'detach':
			wp_media_attach_action( absint($_REQUEST['parent_post_id']), 'detach' );
			break;

		case 'attach':
			wp_media_attach_action( absint($_REQUEST['found_post_id']) );
			break;
	...
	wp_redirect( remove_query_arg( array( '_wp_http_referer', '_wpnonce' ), esc_url( $_SERVER['REQUEST_URI'] ) ) );
	exit;
}
...
if ( current_user_can( 'upload_files' ) ) {
	?>
	<a href="<?php echo esc_url( admin_url( 'admin.php?page=arvancloud-vod-videos-add' ) ); ?>" class="page-title-action"><?php echo esc_html_x( 'Add New', 'file' ); ?></a>
						<?php
}
...
if ( ! empty( $_GET['posted'] ) ) {
	...
	$_SERVER['REQUEST_URI'] = remove_query_arg( array( 'posted' ), esc_url($_SERVER['REQUEST_URI'] ));
}
...
if ( ! empty( $_GET['attached'] ) && absint( $_GET['attached'] ) ) {
	...
	$_SERVER['REQUEST_URI'] = remove_query_arg( array( 'detach', 'attached' ), esc_url($_SERVER['REQUEST_URI'] ));
}
...
if ( ! empty( $_GET['deleted'] ) && absint( $_GET['deleted'] ) ) {
	...
	$_SERVER['REQUEST_URI'] = remove_query_arg( array( 'deleted' ), esc_url($_SERVER['REQUEST_URI'] ));
}
...
if ( ! empty( $_GET['trashed'] ) && absint( $_GET['trashed'] ) ) {
	...
	$message               .= ' <a href="' . esc_url( wp_nonce_url( 'admin.php?page=arvancloud-vod-videos&doaction=undo&action=untrash&ids=' . ( isset( $_GET['ids'] ) ? sanitize_text_field($_GET['ids']) : '' ), 'bulk-media' ) ) . '">' . __( 'Undo' ) . '</a>';
	$_SERVER['REQUEST_URI'] = remove_query_arg( array( 'trashed' ), esc_url($_SERVER['REQUEST_URI'] ));
}
...
if ( ! empty( $_GET['untrashed'] ) && absint( $_GET['untrashed'] ) ) {
	...
	$_SERVER['REQUEST_URI'] = remove_query_arg( array( 'untrashed' ), sanitize_text_field($_SERVER['REQUEST_URI']) );
}

$get_ids = isset($_GET['ids']) ? $_GET['ids'] : 0;
Helper::sanitize_recursive($get_ids, 'absint');


$messages[1] = __( 'Media file updated.' );
$messages[2] = __( 'Media file permanently deleted.' );
$messages[3] = __( 'Error saving media file.' );
$messages[4] = __( 'Media file moved to the Trash.' ) . ' <a href="' . esc_url( wp_nonce_url( 'admin.php?page=arvancloud-vod-videos&doaction=undo&action=untrash&ids=' . ( $get_ids  ), 'bulk-media' ) ) . '">' . __( 'Undo' ) . '</a>';
$messages[5] = __( 'Media file restored from the Trash.' );

if ( ! empty( $_GET['message'] ) && isset( $messages[ $_GET['message'] ] ) ) {

	$message                = $messages[ sanitize_text_field($_GET['message']) ];
	$_SERVER['REQUEST_URI'] = remove_query_arg( array( 'message' ), esc_url($_SERVER['REQUEST_URI']) );
}
